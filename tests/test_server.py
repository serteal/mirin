...
from .helpers import FakeDecoderModel, FakeLlamaModel, FakeTokenizer, get_proxy
...
def test_server_generate_many_map_matches_local_batched_generate() -> None:
    ...
    expected_tokens = cast(
        torch.Tensor,
        expected._model_output if isinstance(expected, ti.Output) else expected,
    )
    ...
    assert torch.equal(torch.cat(actual, dim=0), expected_tokens)
